...
@interface LLImagePreviewController () <UIScrollViewDelegate>

@property (nonatomic) LLScrollView *curShowScrollView;

@property (nonatomic) LLBigSelectView *bigSelectView;
@property (nonatomic) UIScrollView *scrollView;
@property (nonatomic) NSMutableArray<LLScrollView *> *innerScrollViews;
@property (nonatomic) LLPhotoToolbar *toolbar;

@end

@implementation LLImagePreviewController {
    NSInteger length;
...
- (void)setupNavigationBar {
    ...
    _bigSelectView = [[LLBigSelectView alloc] init];
...
- (void)setupViews {
    ...
    for (int i=0; i < length; i++) {
        LLScrollView * _innerScrollView = [[LLScrollView alloc] init];
...
- (void)fillScrollView:(LLScrollView *)scrollView withAssetIndex:(NSInteger)assetIndex {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (int i=0; i<length; i++) {
        LLScrollView *innerScrollView = self.innerScrollViews[i];
...
- (UIView *)viewForZoomingInScrollView:(LLScrollView *)scrollView {
...
- (void)scrollViewDidZoom:(LLScrollView *)scrollView {
...
- (LLScrollView *)scrollViewWithAssetIndex:(NSInteger)assetIndex {
    for (LLScrollView *scrollView in self.innerScrollViews) {
