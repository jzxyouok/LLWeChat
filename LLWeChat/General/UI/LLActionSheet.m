...
- (void)showInWindow:(UIWindow *)window {
    [self showInWindow:window windowBounds:[LLUtils screenFrame]];
}

- (void)showInWindow:(UIWindow *)window windowBounds:(CGRect)windowBounds {
    if (!actionSheetContainer) {
        actionSheetContainer = [[UIView alloc] init];
        actionSheetContainer.backgroundColor = [UIColor colorWithWhite:0.f alpha:0.7f];
    }
    
    if (!actionSheetContainer.superview){
        if (window == [LLUtils popOverWindow]) {
