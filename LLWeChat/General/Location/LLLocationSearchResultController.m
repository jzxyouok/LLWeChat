...
- (void)viewDidLoad {
    ...
    _tableView = [[UITableView alloc] initWithFrame:SCREEN_FRAME style:UITableViewStylePlain];
...
- (void)onPOISearchDone:(AMapPOISearchBaseRequest *)_request response:(AMapPOISearchResponse *)response {
    NSLog(@"搜索结果返回: %ld %ld", _request.offset, response.count);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if ((_tableView.tableFooterView) && (scrollView.contentOffset.y + scrollView.frame.size.height + 2 >= scrollView.contentSize.height)) {
        [self setFooterViewIsLoading:YES];
    ...
    for (NSInteger i = 0, r = text.length; i <= r; i++) {
        ...
        if (isOK) {
            ...
            if (isMatch) {
                ...
                [attributedString addAttribute:NSForegroundColorAttributeName value:kLLTextColor_green range:range];
            }
        }
