...
+ (NSString *)sizeStringWithStyle:(id)style size:(NSInteger)size {
    if (size < 1024 * 1024) {
        return [NSString stringWithFormat:@"%ldK", size/1024];
    }else {
...
+ (NSMutableAttributedString *)parseText:(NSMutableAttributedString *)test {
    NSError *error = nil;
    NSDataDetector *detector = [NSDataDetector dataDetectorWithTypes:NSTextCheckingTypeLink|NSTextCheckingTypePhoneNumber
                                                               error:&error];
    NSArray *matches = [detector matchesInString:test.string
                                         options:kNilOptions
                                           range:NSMakeRange(0, [test length])];
    
    for (NSTextCheckingResult *match in matches) {
        NSRange matchRange = [match range];
        [test addAttribute:NSForegroundColorAttributeName value:kLLTextLinkColor range:matchRange];
    }
