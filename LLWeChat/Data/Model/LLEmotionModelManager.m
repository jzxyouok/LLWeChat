...
- (NSRange)rangeOfEmojiAtEndOfString:(NSString *)string {
    ...
    if (range.location != NSNotFound) {
        NSLog(@"range ：%@", [string substringWithRange:range]);
    }else {
        NSLog(@"not found");
    }
...
- (NSMutableAttributedString *)convertToEmotionWithAttachment:(NSString *)text font:(UIFont *)font {
    NSError *error;
    ...
    [attributeString addAttributes:dict range:NSMakeRange(0, attributeString.length)];
    ...
    while(YES) {
        ...
        attachMent.image = [self imageForEmotionPNGName:imageName];
    ...
    if (needHack) {
        ...
        if (!str) {
            NSTextAttachment *attachMent = [[NSTextAttachment alloc] init];
            attachMent.image = [UIImage imageWithColor:[UIColor redColor]];
            
            CGFloat height = font.lineHeight;
            attachMent.bounds = CGRectMake(0, -4, 0, height);
