...
- (void)viewDidLoad {
    ...
    self.tableView = [[UITableView alloc] initWithFrame:SCREEN_FRAME style:UITableViewStyleGrouped];
    ...
    if (!cell) {
        ...
        switch (indexPath.section) {
            ...
            case 1: {
                ...
                LLPushDisplayStyle style = [LLPushOptions sharedOptions].displayStyle;
                [cell setSwitchOn:(style == kLLPushDisplayStyleMessageSummary) animated:YES];
                break;
            }
            case 2:
            ...
            case 3: {
                cell.accessoryType_LL = kLLTableViewCellAccessorySwitch;
                if (indexPath.row == 0) {
                    BOOL isEnabledAlertSound = [LLPushOptions sharedOptions].isEnabledAlertSound;
                    [cell setSwitchOn:isEnabledAlertSound animated:NO];
                }else if (indexPath.row == 1) {
                    BOOL isEnabledVibrate = [LLPushOptions sharedOptions].isEnabledVibrate;
            ...
            case 4: {
                cell.accessoryType_LL = kLLTableViewCellAccessorySwitch;
                BOOL isEnabledMomentUpdate = [LLPushOptions sharedOptions].isEnabledMomentsUpdates;
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    if (![self.navigationController.childViewControllers containsObject:self]) {
        ...
        [LLPushOptions sharedOptions].displayStyle = cell.isSwitchOn ? kLLPushDisplayStyleMessageSummary : kLLPushDisplayStyleSimpleBanner;
        
        cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:3]];
        [LLPushOptions sharedOptions].isEnabledAlertSound = cell.isSwitchOn;
        
        cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:1 inSection:3]];
        [LLPushOptions sharedOptions].isEnabledVibrate = cell.isSwitchOn;
