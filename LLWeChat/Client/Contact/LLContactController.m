...
@property (nonatomic) NSMutableArray<NSMutableArray<LLUserModel *> *> *dataArray;
@property (nonatomic) NSMutableArray<LLUserModel *> *contactSource;
...
- (void)fetchData {
    WEAK_SELF;
    [[LLContactManager sharedManager] asynGetContactsFromServer:^(NSArray<LLUserModel *> *contacts) {
        [weakSelf processData:contacts];
    }];
}

- (void)processData:(NSArray<LLUserModel *> *)contacts {
    ...
    NSMutableArray<NSMutableArray<LLUserModel *> *> *sortedArray = [NSMutableArray arrayWithCapacity:highSection];
    for (int i = 0; i < highSection; i++) {
        NSMutableArray<LLUserModel *> *sectionArray = [NSMutableArray arrayWithCapacity:1];
        [sortedArray addObject:sectionArray];
    }
    
    //按首字母分组
    for (LLUserModel *model in self.contactSource) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        LLUserModel *userModel = self.dataArray[indexPath.section - 1][indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        LLUserModel *model = self.dataArray[indexPath.section-1][indexPath.row];
