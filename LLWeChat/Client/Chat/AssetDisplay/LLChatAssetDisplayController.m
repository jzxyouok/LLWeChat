...
//#define BOTTOM_BAR_STYLE_None 1
//
//#define BOTTOM_BAR_STYLE_Image_Show 2
//
//#define BOTTOM_BAR_STYLE_Image_Hide 3
...
//    screenCenter = CGPointMake(SCREEN_WIDTH/2, SCREEN_HEIGHT/2);
 ...
 //   self.videoPlayerItems = [NSMutableSet setWithCapacity:length];
...
- (void)layoutAssetViews:(CGSize)size {
    ...
    self.scrollView.contentOffset = CGPointMake(scroll_width * _curAssetView.assetIndex, 0);
    
    if (_imageBottomBar) {
        _imageBottomBar.frame = CGRectMake(0, size.height - BOTTOM_BAR_HEIGHT, size.width, BOTTOM_BAR_HEIGHT);
    }
    
    if (_videoBottomBar) {
        _videoBottomBar.frame =CGRectMake(0, size.height - BOTTOM_BAR_HEIGHT, size.width, BOTTOM_BAR_HEIGHT);
    }
...
- (void)viewWillLayoutSubviews {
    ...
    [self layoutAssetViews:self.view.bounds.size];
}

- (void)viewDidLayoutSubviews {
    [super viewDidLayoutSubviews];
    
    _scrollView.contentSize = CGSizeMake(_allAssets.count * scroll_width, self.view.bounds.size.height);
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if ([_curAssetView isKindOfClass:[LLChatImageScrollView class]]) {
        ...
        _curAssetView.hidden = YES;
        [self.view addSubview:self.originViewSnapshot];
        CGRect fromRect = [[LLUtils currentWindow] convertRect:self.originalWindowFrame toView:self.view];
        self.originViewSnapshot.frame = fromRect;
        ...
        [UIView animateWithDuration:DEFAULT_DURATION
                         ...
                         animations:^{
                                self.originViewSnapshot.frame = SCREEN_FRAME;
                            }
                         completion:^(BOOL finished){
                             _curAssetView.hidden = NO;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (assetIndex + 1 < _allAssets.count && [self assetViewWithAssetIndex:assetIndex + 1] == nil) {
        [self hideAssetViewWithAssetIndex:assetIndex - 2];
        [self showAssetViewWithAssetIndex:assetIndex + 1];
    }else if (assetIndex - 1 >= 0 && [self assetViewWithAssetIndex:assetIndex - 1] == nil) {
        [self hideAssetViewWithAssetIndex:assetIndex + 2];
        [self showAssetViewWithAssetIndex:assetIndex - 1];
    }
...
- (void)showBottomBar {
    [_videoBottomBar removeFromSuperview];
    [_imageBottomBar removeFromSuperview];
    
    if ([_curAssetView isKindOfClass:[LLChatImageScrollView class]]) {
        [self.view addSubview:self.imageBottomBar];
        
    }else if([_curAssetView isKindOfClass:[LLVideoDisplayView class]]){
        [self.view addSubview:self.videoBottomBar];
...
- (void)playerRateDidChanged:(NSURL *)videoURL currentRate:(float)rate {
    if (rate != 0.f) {
        //VideoLayer在初次获取到AVPlayer之后，可能前几帧尚未有图像，会导致黑屏
        //此处延迟1/10秒
        WEAK_SELF;
