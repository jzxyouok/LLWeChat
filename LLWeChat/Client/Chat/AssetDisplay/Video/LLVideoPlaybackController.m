...
- (void)viewDidLoad {
    ...
    self.playStopButtonView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.35];
    self.sliderView.backgroundColor = self.playStopButtonView.backgroundColor;
    self.moreButtonView.backgroundColor = self.playStopButtonView.backgroundColor;
...
- (void)setBackgroundViewVisible:(BOOL)visible {
    self.playStopButtonView.hidden = !visible;
    self.sliderView.hidden = !visible;
    self.moreButtonView.hidden = !visible;
}
...
- (BOOL)isControlViewHidden {
    return self.scrubber.hidden || (self.playButton.hidden && self.stopButton.hidden) || self.totalTimeLabel.hidden || self.currentTimeLabel.hidden;
}
...
- (NSString *)convertTimetoString:(NSInteger)time {
    ...
    NSString *timeString = [NSString stringWithFormat:@"%02ld:%02ld", minutes, seconds];
    return timeString;
}
