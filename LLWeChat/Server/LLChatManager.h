...
- (void)asyncDownloadMessageThumbnail:(LLMessageModel *)model
                           completion:(void (^ __nullable)(LLMessageModel *messageModel, LLError *error))completion;

/**
 *  消息失败重发，会导致其MessageStatus立马由Failed变为Delivering
 *
 */
- (void)resendMessage:(LLMessageModel *)model
             progress:(void (^ __nullable)(LLMessageModel *model, int progress))progress
           completion:(void (^ __nullable)(LLMessageModel *model, LLError *error))completion;
