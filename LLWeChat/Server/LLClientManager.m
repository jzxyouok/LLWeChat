...
- (void)loadPushOptionsFromServer {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (!error) {
            [LLPushOptions sharedOptions].displayStyle = (LLPushDisplayStyle)pushOptions.displayStyle;
            [LLPushOptions sharedOptions].noDisturbSetting = (LLPushNoDisturbSetting)pushOptions.noDisturbStatus;
            [LLPushOptions sharedOptions].noDisturbingStartH = pushOptions.noDisturbingStartH;
            [LLPushOptions sharedOptions].noDisturbingEndH = pushOptions.noDisturbingEndH;
            
            NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
            
            id setting = [userDefaults objectForKey:PUSH_OPTIONS_VIBRATE_KEY];
            if (setting) {
                [LLPushOptions sharedOptions].isEnabledVibrate = [setting boolValue];
                [LLPushOptions sharedOptions].isEnabledAlertSound = [[userDefaults objectForKey:PUSH_OPTIONS_SOUND_KEY] boolValue];
            }else {
                [LLPushOptions sharedOptions].isEnabledVibrate = YES;
                [LLPushOptions sharedOptions].isEnabledAlertSound = YES;
                [userDefaults setObject:@(YES) forKey:PUSH_OPTIONS_VIBRATE_KEY];
                [userDefaults setObject:@(YES) forKey:PUSH_OPTIONS_SOUND_KEY];
...
- (void)savePushOptionsToServer {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        LLPushOptions *llPushOptions = [LLPushOptions sharedOptions];
        
        NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
        [userDefaults setObject:@(llPushOptions.isEnabledVibrate) forKey:PUSH_OPTIONS_VIBRATE_KEY];
        [userDefaults setObject:@(llPushOptions.isEnabledAlertSound) forKey:PUSH_OPTIONS_SOUND_KEY];
        [userDefaults synchronize];
